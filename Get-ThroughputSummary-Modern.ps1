...
---------------------------------------------------------------------
0.1 	- Initial release
0.2     - Updated to support the new Azure AD Module
...
.DESCRIPTION
Script to produce throughput report based on the move requests within an Office 365 tenant.
...
$TenantName = $tenant.VerifiedDomains | Where-Object {($_.Name -like "*.onmicrosoft.com") -and ($_.Name -notlike "*mail*")} | Select Name

$tenantID = $TenantName.Name.Split(".")[0]
